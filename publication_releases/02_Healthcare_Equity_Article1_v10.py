...
def _run_article1_joint_burden(
    ...
    clinical_groups = np.asarray(artifact.demo_test)

    # Article 1 v10: keep the clinical sensitive attribute separate from the
    # simulated structural-access group. The latter is an independent, balanced
    # binary stress-test assignment and must not be interpreted as an observed
    # demographic-geographic relationship in Nigeria.
    n_patients = len(clinical_groups)
    structural_rng = np.random.default_rng(int(seed) + 1701)
    structural_access_groups = structural_rng.integers(0, 2, size=n_patients)
    ...
    assigned = []
    for idx, structural_g in enumerate(structural_access_groups):
        use_structural = u_structural[idx] < cfg.group_location_coupling
        if use_structural:
            pool = worse_access if int(structural_g) == 0 else better_access
    ...
    rec = pd.DataFrame({
        ...
        "group": clinical_groups,  # backward-compatible clinical subgroup
        "clinical_sensitive_group": clinical_groups,
        "structural_access_group": structural_access_groups,
...
def _summarize_article1_coupling_scenario(summary: dict, records: pd.DataFrame, coupling: float) -> list[dict]:
    ...
    if d.empty:
        return []
    rows = []
    group_col = "structural_access_group" if "structural_access_group" in d.columns else "group"
    group_values = list(d[group_col].drop_duplicates())
    for group_value in group_values + ["ALL"]:
        g = d if group_value == "ALL" else d[d[group_col] == group_value]
        if g.empty:
            continue
        fnr = float(g["missed_referral_opportunity_m"].mean())
        ...
        rows.append({
            ...
            "group": str(group_value),
            "group_definition": "simulated_structural_access_group" if group_col == "structural_access_group" else "clinical_sensitive_group",
            "n_referral_eligible": int(len(g)),
            "fnr_missed_referral": fnr,
            "geographic_constraint": constraint,
            "joint_burden_j": joint,
